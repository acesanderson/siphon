...
class ImageEnricher(EnricherStrategy):
    """Enrich image vision description with LLM. Summary uses RoutingSummarizer."""
    ...
    async def enrich(
        ...
        description_str = self.prompt_loader["image_description"].render(input_variables)
    ...
    async def _summarize(self, text: str, metadata: dict[str, Any]) -> str:
        ...
        text_input = _TextInput(data=text, source_id="image", guideline=guideline)
