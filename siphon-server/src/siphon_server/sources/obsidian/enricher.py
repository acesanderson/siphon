...
class ObsidianEnricher(EnricherStrategy):
    """Enrich Obsidian note cluster content with LLM. Summary uses RoutingSummarizer."""
    ...
    async def enrich(
        ...
        description_str = self.prompt_loader["obsidian_description"].render(input_variables)
    ...
    async def _summarize(self, text: str, metadata: dict[str, Any]) -> str:
        ...
        text_input = _TextInput(data=text, source_id="obsidian", guideline=guideline)
