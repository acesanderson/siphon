...
class AudioEnricher(EnricherStrategy):
    """Enrich audio transcript content with LLM. Summary uses RoutingSummarizer."""
    ...
    async def enrich(
        ...
        if content.source_type != self.source_type:
            raise ValueError(
                f"AudioEnricher can only enrich content of type {self.source_type}, "
                f"got {content.source_type} instead."
            )
        ...
        description_str = self.prompt_loader["audio_description"].render(input_variables)
    ...
    async def _summarize(self, text: str, metadata: dict[str, Any]) -> str:
        ...
        text_input = _TextInput(data=text, source_id="audio", guideline=guideline)
